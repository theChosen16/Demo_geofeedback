...
- [x] **Servicio API configurado y corregido**:
  ...
  - ✅ **CRÍTICO**: config.py corregido para usar `hostaddr` (bypass DNS IPv6)
  - 🔄 Redesplegando automáticamente vía GitHub push (commit 9b2b1b6)
...
3. **Conectividad IPv6 a Supabase (Build #3-4)**:
   ...
   - **Causa Raíz**: psycopg2/libpq realiza su propia resolución DNS cuando se usa el parámetro `host`, incluso si se pasa una IP
   - **Solución Inicial (Fallida)**: Resolver a IPv4 y pasar a `host` → No funcionó, libpq seguía haciendo DNS lookup
   - **Solución Final**: Usar parámetro `hostaddr` en lugar de `host`:
     - `hostaddr`: Acepta IP numérica y bypasea DNS completamente ✅
     - `host`: Se mantiene para autenticación SSL
     - Agregar función `resolve_ipv4()` que usa `socket.AF_INET`
     - Cambio en línea 58: `'hostaddr': resolved_ip` + `'host': url.hostname`
     - Agregar logging para debug
   - **Estado**: ✅ Resuelto (Commits 27e91e5, 9b2b1b6)
